...
let plpSecondaryImages = setInterval(function () {
    let productWraps = document.querySelectorAll('.product-item') // get product cards
    if (!productWraps.length) {
        return
    }
    clearInterval(plpSecondaryImages)
    productWraps.forEach(function (productWrap) {
        let pdpLink = productWrap.querySelector('a').href // get PDP links
        let swatches = productWrap.querySelectorAll('.product-quick-add .colour-option li')
        let images = Array.from(swatches).map(swatch => swatch.getAttribute('data-image'))

        if (!images || images.length < 1) { // If there aren't any secondary images then don't bother making it a carousel
            return
        }
        let imagesHtml = ``
        for (let i = 0; i < images.length; i++) { // Can increase or decrease the number here to get more or less images in the carousel. BEWARE THAT MORE IMAGES = SLOWER LOAD TIMES!
            imagesHtml += `<a href="${pdpLink}"><img src="${images[i]}" /></a>` // wrap the images in links so they can click each slide of the carousel
        }

        let imageWrapper = productWrap.querySelector('.product-image-wrap') // get the element currently wrapping the image
    ...
    $('.product-quick-add .colour-option li').on('click', function () {
        let background = $(this).attr('data-image')
        let carousel = $(this).closest('.product-item').find('.slick-slider')
