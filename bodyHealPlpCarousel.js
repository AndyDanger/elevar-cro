...
var plpSecondaryImages = setInterval(function () {
    var productWraps = document.querySelectorAll('.product__grid-item') // get product cards
    if (!productWraps.length) {
        return
    }
    clearInterval(plpSecondaryImages)
    productWraps.forEach(function (productWrap) {
        var pdpLink = productWrap.querySelector('.product-thumbnail__title').href // get PDP links
        ...
        request.done(function () {
            ...
            var imageWrapper = productWrap.querySelector('.image-element__wrap') // get the element currently wrapping the image
