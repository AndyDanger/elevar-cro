...
window.newDesktopGrid = 3
window.oldMobileGrid = 0
window.newMobileGrid = 2
...
var moreProductsInterval = setInterval(function() {
    ...
    $('[data-grid-count="2"], [data-grid-count="4"]').click(function() {
        ...
        var tracker = ga.getAll().find(tracker => tracker.model.data.ea[':trackingId'] == 'UA-34408006-1')
        if (tracker) {
            tracker.send('event', 'collection page', 'grid filter click - ' + window.newMobileGrid, window.location.href)
        }
        addNewStyle()
    })
    ...
    $('[data-grid-count="3"], [data-grid-count="5"], [data-grid-count="8"]').click(function() {
        ...
        var tracker = ga.getAll().find(tracker => tracker.model.data.ea[':trackingId'] == 'UA-34408006-1')
        if (tracker) {
            tracker.send('event', 'collection page', 'grid filter click - ' + window.newDesktopGrid, window.location.href)
        }
        addNewStyle()
    })
...
function addNewStyle() {
    ...
    #PageContainer .isp_grid_product:hover form.isp_add_to_cart_form {
top: unset !important;
