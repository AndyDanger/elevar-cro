...
let variant1 = () => {
    let variantInterval = setInterval(() => {
        let thing = document.querySelector(`.section-banner-carousel .slick-initialized`)
        if (!thing) return
        clearInterval(variantInterval)
        document.body.classList.add(`v1`)
        $(thing).slick(`unslick`)
        let slides = thing.querySelectorAll(`.section-banner-image`)
        slides[1].remove()
        slides[2].remove()
    }, 250)
}
...
let variant2 = () => {
    let variantInterval = setInterval(() => {
        ...
        $(thing).slick(`unslick`)
        let slides = thing.querySelectorAll(`.section-banner-image`)
        slides[0].parentElement.insertBefore(slides[1], slides[0])
        $(thing).slick()
    }, 250)
}
